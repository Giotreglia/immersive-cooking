...
export class EditRecipeComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private router: Router, private backend: BackendService, private ngxCsvParser: NgxCsvParser, private route: ActivatedRoute) {}
  ...
  getRecipe() {
    ...
    this.backend.getRecipe(this.formData).subscribe((resp) => {
      ...
      for (let i = 0; i < this.selectedRecipe.recipeIngredients.length; i++) {
        const ingredient = this.selectedRecipe.ingredients[i];
        ingredient.weight = this.selectedRecipe.recipeIngredients[i].weight;
 ...
 nextStep() {
  ...
  getTotalFootprint() {
    ...
    this.selectedRecipe.footprint_score = 10 - (total / 10);
    if (this.selectedRecipe.footprint_score < 0) {
      this.selectedRecipe.footprint_score = 0;
    }
          ...
          this.backend.s3Upload(requestUrl, this.file, this.file.type).subscribe(
            response => {
              ...
              if(response.status==200 || response.status== 201) {
                ...
                this.recipeData.append('name', this.selectedRecipe.name);
                this.recipeData.append('portion_number', this.selectedRecipe.portion_number);
                this.recipeData.append('portion_unit', this.selectedRecipe.portion_unit);
                this.recipeData.append('description', this.selectedRecipe.description);
                this.recipeData.append('footprint_score', this.selectedRecipe.footprint_score);
                console.log(this.recipeData);
                this.backend.editRecipe(this.recipeData, this.selectedRecipeId).subscribe(
                  response => {
                    let recipe;
